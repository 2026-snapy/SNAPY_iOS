...
final class AlbumService {
    ...
    func fetchLikes(albumId: Int) async throws -> [AlbumLikeUserData] {
        let response = try await requestWithRefresh(.fetchLikes(albumId: albumId))
        guard (200..<300).contains(response.statusCode) else {
            let msg = extractErrorMessage(from: response.data, statusCode: response.statusCode)
            throw AlbumError.serverError(msg)
        }
        do {
            let decoded = try JSONDecoder().decode(AlbumLikeListResponse.self, from: response.data)
            guard decoded.success, let data = decoded.data else {
                throw AlbumError.serverError(decoded.message)
            }
            return data
